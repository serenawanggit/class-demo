	...
	<div id="add-main" style="display: none;margin-top:20px;">
	  ...
	  <form class="layui-form" lay-filter="component-form-group" id="addCourseForm">
        ...
        <div class="layui-form-item">
          ...
          <div class="layui-input-block"><input name="detail" id="detail" lay-verify="title" autocomplete="off" style="width: 200px" placeholder="请输入" class="layui-input"></div>
        </div>
	...
	<script>
		layui.use(['table','form'], function(){
		    ...
		    var basePath = protocol +"//"+host+"/"+contextPath;
		  	...
		  	$.ajax({
		     	type:"POST",
		     	url: basePath + "/teacher/queryCourse",
		     	dataType: "json",
		  	...
		  	table.render({
			    ...
			    ,id: "teacherTableId"
			    ,height: 450
			    ,url: '${pageContext.request.contextPath }/teacher/query' //数据接口
			    ,page: true //开启分页
			    ,cols: [[{checkbox: true}
			      ,{field: 'id', title: '教师编号', width:120}
			      ,{field: 'name', title: '教师名字'}
			      ,{field: 'course', title: '负责课程'}
			    ...
			    if(obj.event === 'edit'){
			    	$.ajax({
				      	...
				      	form.val("component-form-group", { "courseId": msg.data[0].courseId});
				        	...
				        	$("#detail").val(msg.data[0].detail);
				        	$("#id").val(msg.data[0].id);
					       	layer.open({ btn:['提交','取消'], type: 1, title:"课程编辑", closeBtn: false,
						        ...
						        yes:function(index){
						        	...
						        	if(name==null || name==""){
						        		...
						        		$.ajax({
									      	type:"POST",
									      		url:"${pageContext.request.contextPath }/teacher/updateCourseById",
									      	data:{name:name, courseId:courseId,detail:detail,id:id},
									      	dataType: "json",
									      	success: function(msg){
			  	...
			  	switch(obj.event){
 				    	...
 				    	layer.open({
					        ...
					        yes:function(index){
					        	...
					        	if(name==null || name==""){
					        		...
					        		$.ajax({
								      	type:"POST",
								      	url:"${pageContext.request.contextPath }/teacher/addCourse",
								      data:{name:name, detail:detail, courseId:courseId},
								      	dataType: "json",
								      	success: function(msg){
								      		table.reload('teacherTableId', 
								      			{ url: "${pageContext.request.contextPath }/teacher/query" });
				    ...
				    case 'delete':
				    	layer.confirm('确定删除？', {
							...
							var checkStatus = table.checkStatus('teacherTableId'), data = checkStatus.data;
					    	var idArr = [];
						    ...
						    ids = ids.replace("[","").replace("]","");
					      	$.ajax({
						      	type:"POST",
						      		url:"${pageContext.request.contextPath }/teacher/delTeacherById",
						      	data:{ids: ids},
						      	dataType: "json",
						      	success: function(msg){
						      		table.reload('teacherTableId', { url: '/class-demo/tacther/query' });
				        			layer.close(index);
						      	}
					      	});
							layer.close(index);
